...
const LevelPips: React.FC<{ level: number }> = ({ level }) => (
    <span className="inline-flex items-center gap-1" aria-hidden>
        {[1, 2, 3].map((i) => (
            <span
                key={i}
                className={`h-1.5 w-3.5 rounded-full ${i <= level ? 'bg-sky-400' : 'bg-slate-700'}`}
            />
        ))}
    </span>
);
...
export const DecodingLab: React.FC<DecodingLabProps> = ({ data, dir, speechLocale }) => {
    ...
    return (
        <div className="rounded-2xl border border-sky-500/30 bg-slate-900/50 p-5 text-start" dir={dir}>
            {/* כותרת */}
            ...
            <div className="mb-4 rounded-xl border border-slate-700/50 bg-slate-950/40 p-3">
                ...
                <p className="text-sm font-bold text-slate-100">{data.promptBase}</p>
            </div>

            {/* בורר סגנון הבחירה (מצב A) */}
            <div className="mb-1 text-[11px] font-bold uppercase tracking-wider text-slate-500">{data.styleLabel}</div>
            ...
            <motion.div
                ...
                className="mb-5 rounded-xl border border-sky-500/25 bg-sky-950/15 p-3"
            >
                ...
                <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2">
                    <span className="inline-flex items-center gap-2 text-[12px] font-bold text-slate-300">
                    ...
                    return (
                        <div
                            ...
                            className={`rounded-xl border p-3 transition-colors ${isChosen
                                ? 'border-sky-400/60 bg-sky-950/20 ring-1 ring-sky-400/30'
                                : 'border-slate-700/50 bg-slate-950/30'
                                }`}
                        >
                            <div className="mb-2 flex items-center justify-between gap-2">
                                <span className={`inline-flex items-center gap-1.5 text-sm font-bold ${isChosen ? 'text-sky-100' : 'text-slate-200'}`}>
                                    {isChosen && <Crown size={14} className="text-sky-300" aria-hidden />}
                                    {c.label}
                                </span>
                                <span className={`font-mono text-xs tabular-nums ${isChosen ? 'font-bold text-sky-200' : 'text-slate-400'}`} dir="ltr">{c.prob}%</span>
                            </div>
                            <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-800/80">
                                <motion.div
                                    ...
                                    className={`h-full rounded-full ${isChosen ? 'bg-sky-400' : 'bg-slate-500'}`}
                                />
                            </div>
                        </div>
                    );
            ...
            <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={reduce ? { duration: 0 } : { duration: DUR.quick }}
                className="mt-5 rounded-xl border border-sky-500/30 bg-sky-950/15 p-4"
            >
                <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-2">
                        <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">{data.selectedLabel}</span>
                        <span className="inline-flex items-center gap-1.5 rounded-full border border-sky-400/50 bg-sky-900/30 px-3 py-1 text-sm font-black text-sky-100">
            ...
            <button
                ...
                className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-sky-500/50 bg-sky-900/25 px-4 py-3 text-sm font-bold text-sky-100 transition-colors hover:bg-sky-900/40 sm:w-auto"
            >
                <Shuffle size={16} aria-hidden /> {data.replayButton}
            </button>

            {/* הבהרות: ההמשכים כביטויים שלמים, וההסתברויות והבחירות כהמחשה בלבד */}
            <p className="mt-4 text-[13px] leading-relaxed text-slate-500">{data.continuationNote}</p>
