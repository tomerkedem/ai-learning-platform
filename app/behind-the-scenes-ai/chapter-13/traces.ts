...
export const NARRATION: Record<string, SectionNarration> = {
    lab: {
        ...
        intro: 'בחיים האמיתיים המשתמש לא כותב "עכשיו תבצע Tokenization". הוא פשוט כותב "בדוק למה החבילה לא הגיעה", ומצפה שהמערכת תבין מה לעשות. המעבדה מראה שהבקשה הפשוטה הזאת מפעילה תהליך שלם. בצד אחד אתם כותבים, בצד השני המנוע נפתח שלב אחר שלב.',
        takeaway: 'בקשה פשוטה אחת מפעילה מנוע שלם. כאן רואים אותו מבפנים.',
        tryThis: 'כתבו בקשה אחת, ועברו בין Chat ל-Agent כדי לראות איך אותו קלט מפעיל שני מנועים שונים. לחצו על שלב בפס כדי לפתוח את הפרטים שלו.',
    },
    chatReplay: {
