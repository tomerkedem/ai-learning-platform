...
export default function BehindTheScenesChapter17() {
    ...
    return (
        <ChapterLayout courseId="behind-the-scenes-ai" currentChapterId={17}>
            ...
            <div className="relative">
                ...
                >
                    ...
                    <div className="relative z-10">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black text-white leading-[1.1] mb-4">
                            {c17.hero.titleLead}{' '}
                            <span className={`${isRtl ? 'bg-gradient-to-l' : 'bg-gradient-to-r'} from-teal-400 via-emerald-400 to-violet-400 bg-clip-text text-transparent`}>
                                {c17.hero.titleHighlight}
                            </span>
                        </h1>

                        <div className="flex items-start gap-2.5 max-w-3xl">
                            <p className="text-lg text-slate-300 leading-relaxed">{c17.hero.lede}</p>
                ...
                {/* המנטור: צ'אט עונה, Agent מתקדם. ממוקם בצד החיצוני לפי כיוון הקריאה. */}
                <div className={`pointer-events-none absolute top-1/2 z-20 hidden w-[248px] -translate-y-1/2 xl:block ${isRtl ? 'left-full ml-3 2xl:ml-6' : 'right-full mr-3 2xl:mr-6'}`}>
                    <Mentor pose="explain-opposite" line={c17.mentor.hero} width={248} flip={!isRtl} />
                </div>
            </div>
                ...
                <div className="rounded-[2rem] border border-slate-700/50 bg-slate-900/50 p-6 backdrop-blur-xl md:p-8">
                    <div className="mb-4 flex items-start justify-between gap-2.5">
                        <div>
                            ...
                            <h3 className="text-xl font-black text-white md:text-2xl">{c17.primer.title}</h3>
                            <p className="mt-1 text-sm font-medium text-slate-400">{c17.primer.subtitle}</p>
                        </div>
            ...
            <section className="mt-12 text-start" dir={dir}>
                <div className="rounded-2xl border border-slate-700/50 bg-slate-900/40 p-5">
                    <div className="mb-3 flex items-center justify-between gap-2">
                        <div className="text-sm font-bold text-slate-100">{c17.see.title}</div>
                    ...
                    <div className="rounded-xl border border-slate-700/50 bg-slate-950/40 p-3.5">
                        <div className="mb-1 text-[11px] font-bold uppercase tracking-wider text-slate-400">{c17.see.goalLabel}</div>
                        <p className="text-[15px] font-bold leading-relaxed text-slate-100">{c17.see.goal}</p>
                    </div>
                    ...
                    <div className="mt-3 grid gap-3 md:grid-cols-2">
                        ...
                        <div className="rounded-xl border border-sky-500/30 bg-sky-950/15 p-4">
                            ...
                            <p className="text-[14px] leading-relaxed text-slate-200">{c17.see.chat}</p>
                        </div>

                        {/* מסלול Agent */}
                        <div className="rounded-xl border border-teal-500/30 bg-teal-950/15 p-4">
                            ...
                            <ol className="flex flex-wrap items-center gap-1.5">
                                {c17.see.agent.map((step, i) => (
                                    <React.Fragment key={step}>
                                        <li className="rounded-lg border border-teal-500/30 bg-slate-950/40 px-2 py-1 text-[13px] font-bold leading-tight text-teal-100">
                                            {step}
                                        </li>
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-slate-400">{c17.see.caption}</p>
                </div>
            </section>

            {/* ══════════ מעבדת צ'אט ל-Agent ══════════ */}
            <section id="chat-to-agent-lab" className="relative mt-12 space-y-5 text-start scroll-mt-24" dir={dir}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h3 className="text-2xl font-bold text-white">{c17.lab.sectionTitle}</h3>
                    </div>
                </div>
                ...
                {/* המנטור: עברו בין המצבים וראו מה משתנה */}
                <div className={`absolute top-1/2 -translate-y-1/2 z-20 hidden xl:block pointer-events-none ${isRtl ? 'right-full mr-3 2xl:mr-6' : 'left-full ml-3 2xl:ml-6'}`}>
                    <Mentor pose="inspect" line={c17.mentor.labExplain} width={160} flip={!isRtl} />
                </div>
            </section>
            ...
            <section className="relative mt-12 text-start" dir={dir}>
                <div className={`absolute top-1/2 -translate-y-1/2 z-20 hidden xl:block pointer-events-none ${isRtl ? 'left-full ml-3 2xl:ml-6' : 'right-full mr-3 2xl:mr-6'}`}>
                    <Mentor pose="reassure" line={c17.mentor.misconception} width={160} flip={!isRtl} />
                </div>
                <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-2xl border border-rose-500/30 bg-rose-950/10 p-5">
                        ...
                        <p className="leading-relaxed text-slate-300">{c17.misconception.wrongQuote}</p>
                    </div>
            ...
            <section className="relative mt-12 text-start" dir={dir}>
                <div className={`absolute top-1/2 -translate-y-1/2 z-20 hidden xl:block pointer-events-none ${isRtl ? 'left-full ml-3 2xl:ml-6' : 'right-full mr-3 2xl:mr-6'}`}>
                    <Mentor pose="pointdown" line={c17.mentor.practical} width={160} flip={!isRtl} />
                </div>
                <InsightBox type="intuition" title={c17.practical.title}>
                    ...
                    <span className="mt-3 block text-sm text-slate-400">{c17.practical.caveat}</span>
                </InsightBox>
            </section>
            ...
            <section className="mt-16 mb-4 text-start" dir={dir}>
                <div className="mx-auto max-w-2xl rounded-3xl border border-teal-500/25 bg-gradient-to-br from-teal-900/15 to-slate-900/40 p-7">
                    ...
                    <div className="mt-2 flex items-start justify-between gap-2.5">
                        <h3 className="text-xl font-black text-white md:text-2xl">{c17.bridge.title}</h3>
