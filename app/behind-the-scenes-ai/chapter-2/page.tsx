    ...
    return (
        <div dir={dir} className="text-start">
            <div className="mb-3 flex items-start justify-between gap-2">
                <p className="text-sm font-bold text-[var(--bts-text-body)]">{d.question}</p>
                <SpeakButton text={speakText} />
            </div>
            <p className="mb-4 rounded-lg border border-[var(--bts-border)] bg-[color-mix(in_oklab,var(--bts-panel-to)_40%,transparent)] p-3 text-sm text-[var(--bts-text-secondary)]">{d.prompt}</p>
            ...
            {answered && (
                <motion.div
                    ...
                    className="mt-4 flex items-start justify-between gap-2 rounded-xl border border-emerald-500/30 bg-[color-mix(in_oklab,color-mix(in_oklab,var(--t-l)_var(--bts-tint-mix),var(--t-d))_calc(15%_-_var(--bts-tint-mix)_*_0.075),transparent)] [--t-d:var(--color-emerald-950)] [--t-l:var(--color-emerald-500)] p-3"
                >
                    <p className="text-sm leading-relaxed text-[var(--bts-text-body)]">{d.explanation}</p>
...
export default function BehindTheScenesChapter2() {
    ...
    return (
        <ChapterLayout courseId="behind-the-scenes-ai" currentChapterId={2} themeAware>
            ...
            <section id="input-lab" className="mt-12 space-y-5 text-start scroll-mt-24" dir={dir}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h3 className="text-2xl font-bold text-[var(--bts-text-primary)]">{c2.inputLab.title}</h3>
                    </div>
                </div>
                ...
                <div className="rounded-2xl border border-cyan-500/30 bg-[color-mix(in_oklab,var(--bts-panel-from)_40%,transparent)] p-5">
                    <div className="mb-3 flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-lg font-bold text-[var(--bts-text-bright)]">{c2.fullInput.title}</h3>
                        </div>
                        <SpeakButton text={`${c2.fullInput.title}. ${c2.fullInput.body}`} />
                    </div>
                    <p className="leading-relaxed text-[var(--bts-text-secondary)]">{c2.fullInput.body}</p>

                    {/* המחשה קומפקטית: מה שרואים + מה שהאפליקציה עשויה לצרף = הקלט למודל */}
                    <div className="mt-4 grid gap-2 sm:grid-cols-3">
                        <div className="rounded-xl border border-[var(--bts-border)] bg-[color-mix(in_oklab,var(--bts-panel-to)_30%,transparent)] p-3 text-sm text-[var(--bts-text-body)]">{c2.fullInput.seen}</div>
                        <div className="rounded-xl border border-cyan-500/30 bg-[color-mix(in_oklab,color-mix(in_oklab,var(--t-l)_var(--bts-tint-mix),var(--t-d))_calc(15%_-_var(--bts-tint-mix)_*_0.075),transparent)] [--t-d:var(--color-cyan-950)] [--t-l:var(--color-cyan-500)] p-3 text-sm text-[var(--bts-text-body)]"><span className="font-bold text-cyan-300" dir="ltr">+ </span>{c2.fullInput.added}</div>
                        <div className="rounded-xl border border-indigo-500/40 bg-[color-mix(in_oklab,color-mix(in_oklab,var(--t-l)_var(--bts-tint-mix),var(--t-d))_calc(20%_-_var(--bts-tint-mix)_*_0.1),transparent)] [--t-d:var(--color-indigo-950)] [--t-l:var(--color-indigo-500)] p-3 text-sm font-bold text-indigo-100"><span dir="ltr">= </span>{c2.fullInput.total}</div>
                    </div>

                    <p className="mt-3 text-[13px] leading-relaxed text-[var(--bts-text-muted)]">{c2.fullInput.caveat}</p>
                </div>
            </section>
                ...
                <div className="rounded-2xl border border-[var(--bts-border)] bg-[color-mix(in_oklab,var(--bts-panel-from)_40%,transparent)] p-5 leading-relaxed text-[var(--bts-text-secondary)]">
                    <div className="mb-3 flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="text-sm font-bold text-[var(--bts-text-bright)]">{c2.everyday.title}</div>
                        </div>
                ...
                <div className="rounded-2xl border border-indigo-500/40 bg-[var(--bts-surface)] p-6">
                    <div className="mb-5 flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-xl font-bold text-[var(--bts-text-primary)]">{c2.lock.title}</h3>
                        </div>
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                        <div className="rounded-xl border border-emerald-500/30 bg-[color-mix(in_oklab,color-mix(in_oklab,var(--t-l)_var(--bts-tint-mix),var(--t-d))_calc(10%_-_var(--bts-tint-mix)_*_0.05),transparent)] [--t-d:var(--color-emerald-950)] [--t-l:var(--color-emerald-500)] p-4">
                            ...
                            <p className="text-sm leading-relaxed text-[var(--bts-text-body)]">{c2.lock.truthText}</p>
                        </div>
                        <div className="rounded-xl border border-rose-500/30 bg-[color-mix(in_oklab,color-mix(in_oklab,var(--t-l)_var(--bts-tint-mix),var(--t-d))_calc(10%_-_var(--bts-tint-mix)_*_0.05),transparent)] [--t-d:var(--color-rose-950)] [--t-l:var(--color-rose-500)] p-4">
                            ...
                            <p className="text-sm leading-relaxed text-[var(--bts-text-body)]">{c2.lock.mistakeText}</p>
                        </div>
                    </div>
