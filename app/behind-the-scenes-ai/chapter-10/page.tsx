    ...
    return (
        <ChapterLayout courseId="behind-the-scenes-ai" currentChapterId={10}>
            ...
            <div className="relative">
                ...
                >
                    ...
                    <div className="relative z-10">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black text-white leading-[1.1] mb-4">
                            {c10.hero.titleLead}{' '}
                            <span className={`${isRtl ? 'bg-gradient-to-l' : 'bg-gradient-to-r'} from-violet-400 via-fuchsia-400 to-rose-400 bg-clip-text text-transparent`}>
                                {c10.hero.titleHighlight}
                            </span>
                        </h1>

                        <div className="flex items-start gap-2.5 max-w-3xl">
                            <p className="text-lg text-slate-300 leading-relaxed">{c10.hero.lede}</p>
                ...
                {/* המנטור בונה תשובה צעד אחר צעד מחלקים והקשר. ממוקם בצד החיצוני לפי כיוון הקריאה. */}
                <div className={`pointer-events-none absolute top-1/2 z-20 hidden w-[280px] -translate-y-1/2 xl:block ${isRtl ? 'left-full ml-3 2xl:ml-6' : 'right-full mr-3 2xl:mr-6'}`}>
                    <Mentor pose="answerBuilder" line={c10.mentor.hero} width={280} flip={!isRtl} />
                </div>
            </div>
                ...
                <div className="rounded-[2rem] border border-slate-700/50 bg-slate-900/50 p-6 backdrop-blur-xl md:p-8">
                    <div className="mb-4 flex items-start justify-between gap-2.5">
                        <div>
                            ...
                            <h3 className="text-xl font-black text-white md:text-2xl">{c10.primer.title}</h3>
                            <p className="mt-1 text-sm font-medium text-slate-400">{c10.primer.subtitle}</p>
                        </div>
                        <SpeakButton text={primerText} speechLocale={speechLocale} />
                    </div>

                    <p className="text-[15px] leading-relaxed text-slate-300 md:text-base">{c10.primer.lead}</p>
            ...
            <section className="mt-12 text-start" dir={dir}>
                <div className="rounded-2xl border border-slate-700/50 bg-slate-900/40 p-5">
                    <div className="mb-3 flex items-center justify-between gap-2">
                        <div className="text-sm font-bold text-slate-100">{c10.see.title}</div>
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-slate-400">{c10.see.caption}</p>
                </div>
            </section>

            {/* ══════════ מעבדת לולאת הייצור ══════════ */}
            <section id="generation-loop-lab" className="relative mt-12 space-y-5 text-start scroll-mt-24" dir={dir}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h3 className="text-2xl font-bold text-white">{c10.sections.labTitle}</h3>
                    </div>
                </div>
                ...
                <div className={`absolute top-1/2 -translate-y-1/2 z-20 hidden xl:block pointer-events-none ${isRtl ? 'right-full mr-3 2xl:mr-6' : 'left-full ml-3 2xl:ml-6'}`}>
                    <Mentor pose="explain" line={c10.mentor.labExplain} width={160} flip={!isRtl} />
                </div>
            </section>
                ...
                <div className="rounded-2xl border border-slate-700/50 bg-slate-900/40 p-5 leading-relaxed text-slate-300">
                    <div className="mb-3 flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="text-sm font-bold text-slate-100">{c10.analogy.title}</div>
                        </div>
            ...
            <section className="relative mt-12 text-start" dir={dir}>
                <div className={`absolute top-1/2 -translate-y-1/2 z-20 hidden xl:block pointer-events-none ${isRtl ? 'left-full ml-3 2xl:ml-6' : 'right-full mr-3 2xl:mr-6'}`}>
                    <Mentor pose="reassure" line={c10.mentor.misconception} width={155} flip={!isRtl} />
                </div>
                <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-2xl border border-rose-500/30 bg-rose-950/10 p-5">
                        ...
                        <p className="leading-relaxed text-slate-300">{c10.misconception.wrongQuote}</p>
                    </div>
            ...
            <section className="relative mt-12 text-start" dir={dir}>
                <div className={`absolute top-1/2 -translate-y-1/2 z-20 hidden xl:block pointer-events-none ${isRtl ? 'left-full ml-3 2xl:ml-6' : 'right-full mr-3 2xl:mr-6'}`}>
                    <Mentor pose="pointdown" line={c10.mentor.practical} width={160} flip={!isRtl} />
                </div>
                <InsightBox type="intuition" title={c10.practical.title}>
                    ...
                    <span className="mt-3 block text-sm text-slate-400">{c10.practical.caveat}</span>
                </InsightBox>
            </section>

            {/* ══════════ מבדק הבנה ══════════ */}
            <section className="mt-10 rounded-2xl border border-indigo-500/30 bg-indigo-950/15 p-5 text-start" dir={dir}><div className="text-xs font-bold text-indigo-300">{cq.nextQuestionLabel}</div><p className="mt-2 text-base leading-relaxed text-slate-200">{cq.transitions[10]}</p></section>
