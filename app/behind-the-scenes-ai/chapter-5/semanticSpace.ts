...
// כל משפט הוא נקודה קבועה (x, y) במישור, כדי שאפשר יהיה לצייר אותו ולמדוד קרבה בעיניים.
// המשפטים עצמם ממגוון עולמות יומיומיים (חלון/חדר, פעילות בבית, מתכון, מזג אוויר) ולא
// סביב עלילה אחת רציפה, כדי שהמפה תלמד קרבה סמנטית ולא תספר סיפור.
//
// שתי הבחנות מנחות:
//   1. קרבה במרחב = קרבה במשמעות. משפטים שהמודל רואה כקשורים יושבים קרוב זה לזה,
//      גם כשהמילים שונות ("החלון לא פתוח" קרוב ל"השאירו אותו סגור מהבוקר"). משפטים
//      לא קשורים רחוקים (מתכון, מזג אוויר).
//   2. קרבה אינה אמת. שני משפטים יכולים לחלוק כמעט את אותן מילים ולשבת קרוב, אבל
//      להיות הפוכים במשמעות בגלל שלילה ("החלון פתוח" מול "החלון לא פתוח").
//
...
/* ════════════════════════════ מרחב המשפטים ════════════════════════════════ */
// אזור אי הנוחות (שמאל-מעלה), אזור מצב החדר נוגע בו (כי "השאירו אותו סגור" קרוב
// במשמעות ל"לא פתוח"), אזור הפעילות היומיומית למטה-ימין, והלא-קשורים מבודדים רחוק
// למטה-שמאל.
// "השאירו אותו סגור מהבוקר" הוא השכן הקרוב ביותר ל"החלון לא פתוח": משמעות דומה, בלי
// אף מילה משותפת. "החלון פתוח" יושב במרחק בינוני, לא צמוד: הוא חולק מילים אך מהפך את
// המשמעות, ולכן אינו יושב באזור אי הנוחות. מלכודת השלילה (מילים כמעט זהות, משמעות
// הפוכה) מודגמת בנפרד בניסוי השלילה, ולא במפה הראשית, כדי שרשימת השכנים הראשית תלמד
// רק את הכלל הבסיסי.
...
export const PHRASES: Phrase[] = [
    ...
    // מצב החדר. "השאירו אותו סגור" צמוד לעוגן אי הנוחות (השכן הקרוב ביותר); "פתוח"
    // מרוחק ממנו, סמוך יותר לעובדות הנייטרליות (מזגן כבוי, צריך לאוורר) כי משמעותו
    // הפוכה מאי הנוחות.
