...
export default function BehindTheScenesChapter9() {
    ...
    const sBridge: ReadAloudSegment = { id: 'bridge', label: t.behindAi.chapterQuiz.nextQuestionLabel, text: t.behindAi.chapterQuiz.transitions[9] };
    ...
    const baseQuiz = behindAiChapterQuizzes[9];
    ...
    return (
        <ChapterLayout courseId="behind-the-scenes-ai" currentChapterId={9}>
            ...
            <div className="relative">
                ...
                >
                    ...
                    <div className="relative z-10">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-800/70 border border-sky-500/30 mb-5">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black text-white leading-[1.1] mb-4">
                            {c9.hero.titleLead}{' '}
                            <span className={`${isRtl ? 'bg-gradient-to-l' : 'bg-gradient-to-r'} from-sky-400 via-cyan-400 to-indigo-400 bg-clip-text text-transparent`}>
                                {c9.hero.titleHighlight}
                            </span>
                        </h1>

                        <div className="flex items-start gap-2.5 max-w-3xl">
                            <p className="text-lg text-slate-300 leading-relaxed">{c9.hero.lede}</p>
                        ...
                        <div className="mt-6 rounded-2xl border border-slate-700/50 bg-slate-950/40 p-4">
                            ...
                            <p className="text-base font-bold text-slate-100">{c9.prompt}</p>
                        </div>
                ...
                <div className="rounded-[2rem] border border-slate-700/50 bg-slate-900/50 p-6 backdrop-blur-xl md:p-8">
                    <div className="mb-4 flex items-start justify-between gap-2.5">
                        <div>
                            ...
                            <h3 className="text-xl font-black text-white md:text-2xl">{c9.primer.title}</h3>
                            <p className="mt-1 text-sm font-medium text-slate-400">{c9.primer.subtitle}</p>
                        </div>
                        <SpeakButton text={primerText} speechLocale={speechLocale} />
                    </div>

                    <p className="text-[15px] leading-relaxed text-slate-300 md:text-base">{c9.primer.lead}</p>
            ...
            <section className="mt-12 text-start" dir={dir}>
                <div className="rounded-2xl border border-slate-700/50 bg-slate-900/40 p-5">
                    <div className="mb-3 flex items-center justify-between gap-2">
                        <div className="text-sm font-bold text-slate-100">{c9.see.title}</div>
                            ...
                            <React.Fragment key={step}>
                                <span className={`rounded-full border px-3 py-1.5 text-sm font-bold ${
                                    i === c9.see.steps.length - 1
                                        ? 'border-sky-400/50 bg-sky-900/20 text-sky-200'
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-slate-400">{c9.see.caption}</p>
                </div>
            </section>
            ...
            <section id="decoding-lab" className="mt-12 space-y-5 text-start scroll-mt-24" dir={dir}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h3 className="text-2xl font-bold text-white">{c9.lab.sectionTitle}</h3>
                    </div>
                </div>

                <div className="flex items-start gap-2.5">
                    <p className="text-base leading-relaxed text-slate-300">{c9.lab.sectionIntro}</p>
                ...
                <div className="rounded-2xl border border-slate-700/50 bg-slate-900/40 p-5 leading-relaxed text-slate-300">
                    <div className="mb-3 flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="text-sm font-bold text-slate-100">{c9.everyday.title}</div>
                        </div>
            ...
            <section className="mt-12 text-start" dir={dir}>
                <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-2xl border border-rose-500/30 bg-rose-950/10 p-5">
                        ...
                        <p className="leading-relaxed text-slate-300">{c9.mistake.wrong}</p>
                    </div>
                    <div className="rounded-2xl border border-emerald-500/30 bg-emerald-950/10 p-5">
                        ...
                        <p className="leading-relaxed text-slate-300">{c9.mistake.right}</p>
                    </div>
                </div>
            </section>
            ...
            {/* ══════════ שמרני מול פתוח ══════════ */}
            <section className="mt-12 text-start" dir={dir}>
                <div className="rounded-2xl border border-sky-500/30 bg-slate-900/40 p-5 leading-relaxed text-slate-300">
                    <div className="mb-3 flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                            <MessageSquare size={18} className="text-sky-300" />
                            <div className="leading-tight">
                                <div className="text-sm font-bold text-slate-100">{c9.how.title}</div>
                                <div className="text-[11px] font-medium uppercase tracking-[0.2em] text-slate-500" dir="ltr">{c9.how.sub}</div>
                            </div>
                        </div>
                ...
                <div className="rounded-2xl border border-sky-500/40 bg-slate-900/60 p-6">
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                        <div className="rounded-xl border border-emerald-500/30 bg-emerald-950/10 p-4">
                            ...
                            <p className="text-sm leading-relaxed text-slate-200">{c9.lock.trueText}</p>
                        </div>
                        <div className="rounded-xl border border-rose-500/30 bg-rose-950/10 p-4">
                            ...
                            <p className="text-sm leading-relaxed text-slate-200">{c9.lock.falseText}</p>
                        </div>
                    </div>
            ...
            <section className="mt-12 text-start" dir={dir}>
                <InsightBox type="intuition" title={c9.practical.title}>
                    ...
                    <span className="mt-3 block text-sm text-slate-400">{c9.practical.caveat}</span>
                </InsightBox>
            </section>
