...
const UnderstandingLock: React.FC = () => {
    const { t, dir } = useT();
    const c14 = t.behindAi.mistakeLearning;
    const lock = c14.lock;
    ...
    return (
        <div dir={dir} className="text-start">
            <div className="mb-3 flex items-center gap-2">
                ...
                <SpeakButton text={lock.question} speechLocale={c14.contentLocale} />
            </div>
...
export default function BehindTheScenesChapter14() {
    ...
    const guessContent: OpeningGuessContent = {
        ...
        exploreHint: c14.guess.exploreHint,
    };
    ...
    const primerText = `${c14.primer.title}. ${c14.primer.subtitle}. ${c14.primer.lead} ${c14.primer.points.map((p) => `${p.title}. ${p.body}`).join(' ')}`;

    // ── דוק האזנה מודרכת: מקטעי הקראה יציבים בלבד (בלי מצב חי של המעבדה, כפתורים,
    // מנטורים או חידון). התוכן נקרא בשפת contentLocale. ──
    const sHero: ReadAloudSegment = { id: 'hero', label: c14.hero.titleHighlight, text: `${c14.hero.titleLead} ${c14.hero.titleHighlight}. ${c14.hero.lede}` };
    const sGuess: ReadAloudSegment = { id: 'guess', label: c14.guess.eyebrow, text: `${c14.guess.title} ${c14.guess.subtitle}` };
    const sPrimer: ReadAloudSegment = { id: 'primer', label: c14.primer.title, text: primerText };
    const sSee: ReadAloudSegment = { id: 'see', label: c14.see.title, text: `${c14.see.title}. ${c14.see.steps.join(', ')}. ${c14.see.caption}` };
    const sLab: ReadAloudSegment = { id: 'lab', label: c14.lab.sectionTitle, text: `${c14.lab.sectionTitle}. ${c14.lab.sectionIntro}` };
    const sWow: ReadAloudSegment = { id: 'wow', label: c14.insight.title, text: `${c14.insight.title}. ${c14.insight.lead} ${c14.insight.body}` };
    const sMisconception: ReadAloudSegment = { id: 'misconception', label: c14.misconception.rightLabel, text: `${c14.misconception.rightLabel}. ${c14.misconception.rightBody}` };
    const sLock: ReadAloudSegment = { id: 'lock', label: c14.lock.title, text: c14.lock.question };
    const sPractical: ReadAloudSegment = { id: 'practical', label: c14.practical.title, text: `${c14.practical.title}. ${c14.practical.lead} ${c14.practical.uses.join(' ')}` };
    const sCaveat: ReadAloudSegment = { id: 'caveat', label: c14.practical.title, text: c14.practical.caveat };
    const sBridge: ReadAloudSegment = { id: 'bridge', label: t.behindAi.chapterQuiz.nextQuestionLabel, text: t.behindAi.chapterQuiz.transitions[14] };
    ...
    const baseQuiz = behindAiChapterQuizzes[14];
    ...
    const localizedQuiz = {
        ...baseQuiz,
        title: c14.quiz.title,
        subtitle: c14.quiz.subtitle,
        startLabel: c14.quiz.startLabel,
        submitLabel: c14.quiz.submitLabel,
        completedTitle: c14.quiz.completedTitle,
        getReviewLinks,
        questions: baseQuiz.questions.map((q) => ({ ...q, ...c14.quiz.byId[q.id as MistakeLearningQuizId] })),
    };

    return (
        <ChapterLayout courseId="behind-the-scenes-ai" currentChapterId={14} themeAware>
            ...
            <div>
                <motion.section
                    ...
                    dir={dir}
                >
                    <div className="absolute -top-16 -right-16 w-56 h-56 bg-fuchsia-500/10 blur-[80px] rounded-full pointer-events-none" />
                    <div className="absolute -bottom-20 -left-10 w-64 h-64 bg-violet-500/10 blur-[90px] rounded-full pointer-events-none" />

                    <div className="relative z-10">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[color-mix(in_oklab,color-mix(in_oklab,var(--bts-surface-elevated)_var(--bts-tint-mix),var(--color-slate-800))_70%,transparent)] border border-fuchsia-500/30 mb-5">
                            <GraduationCap size={14} className="text-fuchsia-400" />
                            <span className="font-mono text-[11px] tracking-widest uppercase text-fuchsia-300" dir="ltr">{c14.hero.badge}</span>
                        </div>

                        <h1 className="text-4xl md:text-5xl font-black text-[var(--bts-text-primary)] leading-[1.1] mb-4">
                            {c14.hero.titleLead}{' '}
                            <span className={`${isRtl ? 'bg-gradient-to-l' : 'bg-gradient-to-r'} from-[color-mix(in_oklab,var(--color-fuchsia-400)_calc(100%_-_var(--bts-tint-mix)_*_0.4),black)] via-[color-mix(in_oklab,var(--color-violet-400)_calc(100%_-_var(--bts-tint-mix)_*_0.4),black)] to-[color-mix(in_oklab,var(--color-amber-400)_calc(100%_-_var(--bts-tint-mix)_*_0.4),black)] bg-clip-text text-transparent`}>
                                {c14.hero.titleHighlight}
                            </span>
                        </h1>

                        <div className="flex items-start gap-2.5 max-w-3xl">
                            <p className="text-lg text-[var(--bts-text-secondary)] leading-relaxed">{c14.hero.lede}</p>
                            <SpeakButton text={`${c14.hero.titleLead} ${c14.hero.titleHighlight}. ${c14.hero.lede}`} className="mt-1" speechLocale={speechLocale} />
                        </div>

                        <p className="mt-4 text-base font-bold text-fuchsia-200">
                            {c14.hero.hook}
                        </p>

                        <div className="flex flex-wrap gap-3 mt-5 text-xs text-[var(--bts-text-muted)]">
                            <span className="inline-flex items-center gap-1.5">
                                <MousePointerClick size={14} className="text-fuchsia-400" /> {c14.hero.chipTry}
                            </span>
                            <span className="inline-flex items-center gap-1.5">
                                <Layers size={14} className="text-violet-400" /> {c14.hero.chipCompare}
                            </span>
                        </div>
            ...
            <section className="mt-12 text-start" dir={dir}>
                <div className="rounded-[2rem] border border-[var(--bts-border)] bg-[color-mix(in_oklab,var(--bts-panel-from)_50%,transparent)] p-6 backdrop-blur-xl md:p-8">
                    <div className="mb-4 flex items-start justify-between gap-2.5">
                        <div>
                            <span className="mb-2 inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-fuchsia-300">
                                <Sparkles size={14} /> {c14.primer.eyebrow}
                            </span>
                            <h3 className="text-xl font-black text-[var(--bts-text-primary)] md:text-2xl">{c14.primer.title}</h3>
                            <p className="mt-1 text-sm font-medium text-[var(--bts-text-muted)]">{c14.primer.subtitle}</p>
                        </div>
                        <SpeakButton text={primerText} speechLocale={speechLocale} />
                    </div>

                    <p className="text-[15px] leading-relaxed text-[var(--bts-text-secondary)] md:text-base">{c14.primer.lead}</p>

                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                        {c14.primer.points.map((pt) => (
            ...
            <section className="mt-12 text-start" dir={dir}>
                <div className="rounded-2xl border border-[var(--bts-border)] bg-[color-mix(in_oklab,var(--bts-panel-from)_40%,transparent)] p-5">
                    <div className="mb-3 flex items-center justify-between gap-2">
                        ...
                        <SpeakButton text={`${c14.see.title}. ${c14.see.steps.join(', ')}. ${c14.see.caption}`} speechLocale={speechLocale} />
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        {c14.see.steps.map((step, i) => (
                            <React.Fragment key={step}>
                                <span className={`rounded-full border px-3 py-1.5 text-sm font-bold ${
                                    i === c14.see.steps.length - 1
                                        ? 'border-fuchsia-400/50 bg-[color-mix(in_oklab,color-mix(in_oklab,var(--t-l)_var(--bts-tint-mix),var(--t-d))_calc(20%_-_var(--bts-tint-mix)_*_0.1),transparent)] [--t-d:var(--color-fuchsia-900)] [--t-l:var(--color-fuchsia-500)] text-fuchsia-200'
                                        : 'border-[var(--bts-border-mid)] bg-[color-mix(in_oklab,var(--bts-panel-to)_40%,transparent)] text-[var(--bts-text-secondary)]'
                                }`}>
                                    {step}
                                </span>
                                {i < c14.see.steps.length - 1 && <FlowArrow size={15} className="text-violet-400" aria-hidden />}
                            </React.Fragment>
                        ))}
                    </div>
                    <p className="mt-3 text-sm leading-relaxed text-[var(--bts-text-muted)]">{c14.see.caption}</p>
                </div>
            </section>
            ...
            <section id="mistake-learning-lab" className="mt-12 space-y-5 text-start scroll-mt-24" dir={dir}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="text-[11px] font-bold uppercase tracking-[0.25em] text-fuchsia-400" dir="ltr">{c14.lab.sectionEyebrow}</div>
                        <h3 className="text-2xl font-bold text-[var(--bts-text-primary)]">{c14.lab.sectionTitle}</h3>
                    </div>
                </div>
                ...
                <div className="flex items-start gap-2.5">
                    <p className="rounded-2xl border border-[var(--bts-border)] bg-[color-mix(in_oklab,var(--bts-panel-from)_40%,transparent)] p-5 leading-relaxed text-[var(--bts-text-secondary)]">
                        {c14.lab.sectionIntro}
                    </p>
                    <SpeakButton text={`${c14.lab.sectionTitle}. ${c14.lab.sectionIntro}`} className="mt-1" speechLocale={speechLocale} />
                </div>
            ...
            <section className="mt-12 text-start" dir={dir}>
                <InsightBox type="intuition" title={c14.insight.title}>
                    <div className="flex items-start justify-between gap-2.5">
                        ...
                        <SpeakButton text={`${c14.insight.title}. ${c14.insight.lead} ${c14.insight.body}`} speechLocale={speechLocale} />
                    </div>
                    <span className="mt-2 block">{c14.insight.body}</span>
                </InsightBox>
            </section>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-2xl border border-rose-500/30 bg-[color-mix(in_oklab,color-mix(in_oklab,var(--t-l)_var(--bts-tint-mix),var(--t-d))_calc(10%_-_var(--bts-tint-mix)_*_0.05),transparent)] [--t-d:var(--color-rose-950)] [--t-l:var(--color-rose-500)] p-5">
                        <div className="mb-2 flex items-center gap-2 text-rose-200">
                            <XCircle size={18} />
                            <span className="text-sm font-bold">{c14.misconception.wrongLabel}</span>
                        </div>
                        <p className="leading-relaxed text-[var(--bts-text-secondary)]">{c14.misconception.wrongQuote}</p>
                    </div>
                    <div className="rounded-2xl border border-emerald-500/30 bg-[color-mix(in_oklab,color-mix(in_oklab,var(--t-l)_var(--bts-tint-mix),var(--t-d))_calc(10%_-_var(--bts-tint-mix)_*_0.05),transparent)] [--t-d:var(--color-emerald-950)] [--t-l:var(--color-emerald-500)] p-5">
                        <div className="mb-2 flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2 text-emerald-200">
                                ...
                                <span className="text-sm font-bold">{c14.misconception.rightLabel}</span>
                            </div>
                            <SpeakButton text={`${c14.misconception.rightLabel}. ${c14.misconception.rightBody}`} speechLocale={speechLocale} />
                        </div>
                        <p className="leading-relaxed text-[var(--bts-text-secondary)]">
                            {c14.misconception.rightBody}
                        </p>
                    </div>
                </div>
            </section>
            ...
            <section className="mt-12 text-start" dir={dir}>
                <div className="rounded-2xl border border-fuchsia-500/40 bg-[var(--bts-surface)] p-6">
                    <div className="mb-5 flex items-center gap-2">
                        ...
                        <h3 className="text-xl font-bold text-[var(--bts-text-primary)]">{c14.lock.title}</h3>
                    </div>
            ...
            <section className="mt-12 text-start" dir={dir}>
                <InsightBox type="intuition" title={c14.practical.title}>
                    <div className="flex items-start justify-between gap-2.5">
                        ...
                        <SpeakButton text={`${c14.practical.title}. ${c14.practical.lead} ${c14.practical.uses.join(' ')} ${c14.practical.caveat}`} speechLocale={speechLocale} />
                    </div>
                    <ul className="mt-3 space-y-2">
                        {c14.practical.uses.map((line) => (
