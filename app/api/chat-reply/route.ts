...
ענה בעברית, קצר (1-3 משפטים), ענייני ומנומס.
...
במקום רק לענות, נסח בקצרה (1-3 משפטים, עברית) מהו הצעד הנכון הבא: לענות ישירות, לבקש מידע חסר,
...
export async function POST(req: Request) {
    ...
    let body: { text?: string; mode?: FlowMode };
    try {
        body = await req.json();
    } catch {
                ...
                const llm = client.messages.stream(
                    {
                        model: MODEL,
                        max_tokens: 320,
                        system: mode === 'agent' ? SYSTEM_AGENT : SYSTEM_CHAT,
