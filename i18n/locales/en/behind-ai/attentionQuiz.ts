...
export const attentionQuiz = {
    ...
    byId: {
        ...
        4: {
            question: 'In the lab we removed "but", and then replaced "never received" with "received late". What does that teach us about attention?',
            options: [
        ...
        5: {
            question: 'The model puts strong weight on "delivered". A user concludes: "so the model checked reality and the package really was delivered." What is inaccurate about that conclusion?',
            options: [
                'There is no mistake, high attention weight proves that the information is true',
                'Attention weighs links inside the text, it does not check facts in the world. High weight on "delivered" only means that this word is important for processing the context',
                'The mistake is that the model did not really put weight on "delivered"',
                'The mistake is that "delivered" can never receive high weight',
            ],
            explanation:
                'High attention weight means the word is relevant to processing the internal context, not that it is true in reality. Attention links parts of the text to one another, it does not verify whether the package was in fact delivered. For that you need an external source, like a tracking tool, not the attention mechanism.',
        },
