...
export const chapter1Visuals = {
    ...
    predict: {
        ...
        eyebrow: 'Quick guess',
        questionLead: 'What will the engine decide for "',
        questionTail: '"?',
        subtitleChat: 'Before you run the read head - guess how the engine will respond.',
        subtitleAgent: 'Before you run the read head - guess what the engine next step will be.',
        guessAria: (label: string) => `Guess: ${label}`,
        correctLead: 'Spot on! The engine really chose "',
        correctTail: '".',
        wrongLead: 'You guessed "',
        wrongMid: '", but the engine chose "',
        wrongTail: '".',
        correctHint: 'You read correctly where the input leans. Now run the read head below and see why - word by word.',
        wrongHint: 'And that is exactly the interesting part: the input leans a different way than you expected. Run the read head below and see how the engine got there, word by word.',
        impossibleChat: 'Note: in Chat the decision is always one of two - answer or ask for clarification. Stopping or using a tool are reserved for Agent. This is exactly where the line between the two runs.',
        impossibleAgent: 'This action is not relevant to the current task, but it is part of the Agent decision box in other scenarios.',
        guessAgain: 'Guess again',
        mentor: {
            think: 'Guess before you reveal',
            celebrate: 'Spot on!',
            reassure: 'A mistake teaches more',
        },
    },
};
