...
export const fullTraceLab: FullTraceLabContent = {
    ...
    sectionIntro: 'עקבו אחר אותה בקשת תיאום פגישה דרך הרכבת הקשר, עיבוד המודל, עיגון, לולאת Agent, בקרה, ביצוע מדומה ואימות. המסלול הראשי פשוט, ופרטים מתקדמים והסתעפויות זמינים לפי בחירה.',
    heading: 'המסלול המלא',
    kicker: 'SCRIPTED EDUCATIONAL TRACE',
    promptLabel: 'בקשת המשתמש',
    prompt: 'ארגן פגישת תכנון פרויקט של 30 דקות לצוות מחר אחר הצהריים, ונסח הזמנה, אבל אל תשלח אותה בלי אישור שלי.',
    disclosure: 'זו הדמיה לימודית מתוסרטת. אין חיבור חי ליומן או להודעות, אין שליחה אמיתית, ואין חשיפה של שרשרת חשיבה נסתרת.',
    ...
    stages: [
        {
            ...
            facts: [
                { label: 'בקשה נוכחית', value: 'תיאום פגישת צוות של 30 דקות וטיוטת הזמנה בלבד' },
        ...
        {
            id: 'model-input', group: 'קלט והקשר', layer: 'product', status: 'required', tab: 'קלט המודל', title: 'רק ההקשר המורכב נמסר למודל',
            ...
            output: 'System: השתמש במקורות עדכניים ואל תבצע שליחה ללא אישור. Goal: תאם פגישה ונסח הזמנה. Constraint: לא לשלוח. Known: משתתפים, 30 דקות, מחר אחר הצהריים. Missing: שעה מדויקת פנויה. Sources: אין עדיין.',
        },
        {
            ...
            input: 'קטע מהקלט: "ארגן ... פגישת תכנון פרויקט".', process: 'הטוקנייזר מפרק חלקים וממפה כל חלק למזהה.', output: 'רצף IDs שניתן לעיבוד מספרי.',
            details: { label: 'דוגמה קטנה בלבד', items: ['"ארגן" ← token ID 4812', '"פגישת" ← token ID 907', '"30" עשוי להיות טוקן נפרד מהמילה "דקות"', 'המספרים הם מזהים בלבד ואינם מכילים משמעות בעצמם'] },
        },
        {
            ...
            input: 'רצף token IDs.', process: 'כל ID ממופה לייצוג, ושכבות המודל משנות את הייצוגים בהתאם להקשר וליחסים סמנטיים.', output: 'ייצוגים הקשריים שמקשרים פגישה, זמינות, משתתפים וגבול אישור.',
        ...
        },
        {
            ...
            input: 'נדרשת עובדה עדכנית על זמינות בלוח הזמנים.', process: 'נתיב ללא מקור נדחה למשימה הזאת. המערכת מעדיפה grounding בכלי; בשאלת ידע פשוטה ייתכן שהשלב יידלג.', output: 'החלטה לבקש הצעת tool call במקום לענות מהדפוסים בלבד.',
            branchReason: 'זמינות ביומן משתנה בזמן ודורשת ראיה עדכנית. אחזור מוסיף ראיה להקשר ואינו משנה את פרמטרי המודל.',
        },
        {
            ...
            input: 'Goal: בדיקת זמינות והכנת טיוטה. State: חסרה שעה מדויקת פנויה. Constraints: אין שליחה. Proposal: calendar.lookup.', process: 'המודל עשוי להציע קריאת כלי מובנית; האפליקציה בודקת שהכלי זמין ומתאים. מצב המשימה נפרד מזיכרון ארוך טווח.',
            output: 'Tool: calendar.lookup. Input: { participants: "צוות הפרויקט", duration: "30 דקות", range: "מחר אחר הצהריים" }. Expected: available time slot.',
        ...
        },
        {
            ...
            input: 'זהות, integration, משאב ופעולה מבוקשת.', process: 'בדיקה: האם הזהות רשאית לקרוא זמינות ביומן הזה? החלטת מדיניות: allowed לקריאה; שליחה עתידית דורשת approval. פעולה חסומה במדיניות לא נפתחת באמצעות אישור אדם.', output: 'קריאת היומן מותרת; שליחת ההזמנה עדיין לא מאושרת.',
        },
        {
            ...
            input: 'calendar.lookup({ participants: "צוות הפרויקט", duration: "30 דקות", range: "מחר אחר הצהריים" }).', process: 'האינטגרציה המדומה מחזירה נתונים בתוך מגבלת הניסיון.', output: 'status: available. slot: 15:00-15:30.',
            facts: [{ label: 'עדכון מצב משימה', value: 'זמן פנוי אותר; אפשר לנסח טיוטה על בסיס התוצאה' }, { label: 'אמינות מקור', value: 'גם ראיה חיצונית יכולה להיות חלקית או שגויה' }],
        },
        {
            ...
            input: 'תוצאת הכלי הוכנסה להקשר המשימה.', process: 'נבדק בגלוי: תואם לזמן שהוחזר מהיומן ✓; לא הומצאה שעה ✓; לא נטען שנשלח ✓; הטיוטה מסומנת כלא נשלחה ✓.',
        ...
        },
        {
            ...
            input: 'טיוטה מעוגנת, policy: approval required.', process: 'ברירת המחדל בהדגמה היא לא לשלוח. הלומד יכול לבחור הסתעפות חינוכית של אישור או דחייה.', output: 'ממתין לאישור. שום הזמנה לא נשלחה.',
        ...
        },
        {
            ...
            input: 'רק אם ניתן approval חינוכי: sendInvite עם הטיוטה.', process: 'האינטגרציה המדומה מדווחת accepted או rejected; האפליקציה מאמתת את הדיווח מול מטרת המשימה.', output: 'סטטוס כן: success, partial, waiting for approval, blocked, denied, stopped או failed.',
    ...
    branches: {
        ...
        missingInfo: { label: 'חסר טווח זמן מתאים', outcome: 'שואלים את המשתמש במקום להמציא זמן, ולאחר התשובה "מחר אחר הצהריים" ממשיכים למסלול הראשי.' },
        knowledge: { label: 'שאלת ידע פשוטה', outcome: 'כלי ו־approval עשויים להידלג כי אין צורך במידע עדכני או פעולה.' },
        toolNeeded: { label: 'זמינות היומן הנוכחית', outcome: 'נדרש כלי כי הזמינות משתנה בזמן.' },
