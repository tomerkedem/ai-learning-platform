...
export const chapter1 = {
    ...
    redesign: {
        ...
        failures: { title: '一つの結果と複数の障害原因', intro: '弱い回答が直ちにモデルの幻覚を意味するわけではありません。考えられる層を選んでください。', groupLabel: '考えられる障害原因', conclusion: '最終回答だけでは正確な原因が分からないことがあります。終点だけでなく経路を調べます。', items: [{ title: '不明確な依頼', body: '利用者の表現にすでに問題がある場合があります。' }, { title: '入力不足', body: '必要な詳細が書かれず、追加もされていません。' }, { title: '誤った製品指示', body: '製品が不完全または不適切な指示を渡しました。' }, { title: '誤った文脈選択', body: '誤った文脈を加えたか、重要な文脈を省きました。' }, { title: 'モデル生成エラー', body: 'モデルが誤った、または根拠のない出力を生成しました。' }, { title: '外部情報の欠落や古さ', body: '外部情報源が使えないか古い状態です。' }, { title: '検索やツールの障害', body: '検索、ツール、連携が失敗しました。' }, { title: '方針や権限による停止', body: '規則または権限が操作を止めました。' }, { title: '整形や表示の問題', body: 'モデル後、画面までの間に出力が崩れました。' }] },
        summary: { title: '覚える三つのこと', points: ['見える回答は、見えないシステム経路の終点です。', '製品とモデルは別の層です。', '問題は複数の場所で始まりうるため、回答だけでなく経路を調べます。'] },
        bridge: { title: '次の自然な疑問', question: '製品がモデルの回答前に何かを組み立てると分かりました。モデルには正確に何が入るのでしょうか？', body: '次章では、見えるメッセージが入力の全部だと決めつけずに入力の封筒を開きます。', cta: 'モデル入力: 実際に何が入るのか' },
    },
    ...
    summary: {
        ...
        understandBody: 'AI エンジンは答えを「知っている」わけではありません - 選択肢を順位づけし、その差で判断します。差が大きければ確信して答え、差が小さければ正しい一歩は、推測ではなく止まって尋ねることです。あなた自身がそれを見ました: 読み取りヘッドは読みながら先頭が入れ替わるのを示し、一つの語が判断全体を決めることがあります。',
        ruleTitle: '実践的なルール',
