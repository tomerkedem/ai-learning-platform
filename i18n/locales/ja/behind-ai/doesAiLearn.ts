...
export const doesAiLearn = {
    ...
    hero: {
        ...
        hook: 'モデルは「図書館は祝日、10時から14時まで開いています」と言い、あなたは「いいえ、出典には祝日の開館時間が示されていません」と答えた。次に新しいチャットを開いたとき、何が起きる？',
    ...
    mentorRespond: {
        guessCorrect:
            '「効いた」と「保存された」を切り分けました。この切り分けは、目の前で修正がうまくいったときこそ難しくなります。同じ切り分けは製品側の記憶にも使えます。次の会話に戻ってくるものは、誰かが保存したから戻るのであって、モデルが学んだからではありません。',
        guessWrong:
            '内側から見ればその感覚は正確です。指摘したら、答えは実際によくなりました。見落としやすいのは、その修正がどこに置かれたかです。モデルそのものではなく、いまの会話の文脈の中にあります。上に戻って、その修正のどこが新しい会話まで届きうるのかを考えてみてください。',
        quizPass:
            'システムが単に覚えていると決めつけず、文脈、製品の記憶、学習を区別できています。この区別が、保存されていないものに頼ってしまう事態を防ぎます。',
        quizFail:
            'ここで混乱するのは自然です。外から見ると、どの層も「モデルが知っている」ように見えるからです。ラボに戻って四つの層を切り替え、修正がどこで消えるのかを確かめてください。',
    },
    ...
    lock: {
        title: '理解度チェック',
        question: '一つのチャットでモデルを訂正した：「出典がなければ祝日の開館時間を書かない」。それから新しいチャットを開いた。最も安全な前提は？',
        options: [
            '基盤モデルはあなたの規則を永久に学んだ。',
    ...
    practical: {
        ...
        uses: [
            ...
            '規則を明示的に書く。例：「出典がなければ祝日の開館時間を作らない。出典に祝日の時間がなければ、確定した時間はないと書く。」',
